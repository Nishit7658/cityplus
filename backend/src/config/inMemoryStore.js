...
let nextComplaintId = 110;
const COMPLAINTS = [
  {
    id: 108,
    category: 'pothole',
    description: 'Major chronic road crater cluster near Sayajigunj Railway Underbridge. Unrepaired despite 3 municipal tenders.',
    ...
    assigned_by_supervisor_id: 902,
    assigned_by_supervisor_name: 'Sayajigunj Zonal Dispatcher',
    photo_url: '/uploads/pothole_sayajigunj_before.svg',
    photo_after_url: null,
    evidence_photos: [
      '/uploads/pothole_sayajigunj_before.svg',
      '/uploads/water_leak_akota_before.svg'
    ],
    total_cycles: 4,
    months_span: 3,
    reopened_count: 2,
    created_at: new Date(Date.now() - 85 * 86400000).toISOString(), // 85 days ago (~2.8 months)
  ...
  },
  {
    id: 103,
    category: 'open_manhole',
    description: 'Sewer manhole lid missing near Karelibaug Muktanand Circle. High hazard for school children & night traffic.',
    ...
    photo_after_url: null,
    evidence_photos: [
      '/uploads/garbage_market_before.svg'
    ],
    total_cycles: 4,
    months_span: 8,
    reopened_count: 1,
    created_at: new Date(Date.now() - 74 * 86400000).toISOString(), // 74 days ago (~2.5 months)
  ...
  },
  {
    id: 109,
    category: 'water_leak',
    description: 'Underground potable pipe leakage causing water stagnation & road sinkhole on Old Padra Road.',
    ...
    months_span: 2,
    reopened_count: 1,
    created_at: new Date(Date.now() - 64 * 86400000).toISOString(), // 64 days ago (~2.1 months)
  ...
  },
  {
    ...
    assigned_by_supervisor_id: null,
    assigned_by_supervisor_name: null,
    photo_url: '/uploads/pothole_sayajigunj_before.svg',
    photo_after_url: '/uploads/pothole_sayajigunj_after.svg',
    evidence_photos: ['/uploads/pothole_sayajigunj_before.svg'],
    reopened_count: 0,
  ...
  },
  {
    ...
    photo_url: '/uploads/water_leak_akota_before.svg',
    photo_after_url: null,
    evidence_photos: ['/uploads/water_leak_akota_before.svg'],
    reopened_count: 0,
  ...
  },
  {
    ...
    evidence_photos: ['/uploads/garbage_market_before.svg'],
    reopened_count: 0,
...
function enrichComplaint(c) {
  ...
  const daysUnresolved = Math.floor((Date.now() - new Date(c.created_at).getTime()) / 86400000);
  const monthsSpan = c.months_span || (spot ? Math.max(1, Math.round((Date.now() - new Date(spot.first_reported_at).getTime()) / (30 * 86400000))) : (daysUnresolved >= 30 ? Math.round(daysUnresolved / 30) : 1));
  const isChronicOverdue = c.status !== 'Resolved' && (daysUnresolved >= 60 || monthsSpan >= 2 || (c.is_recurring && (c.total_cycles || 1) >= 2));
  ...
  return {
    ...
    assigned_by_supervisor_name: c.assigned_by_supervisor_name || (c.assigned_officer_id ? 'Sayajigunj Zonal Dispatcher' : null),
    assigned_by_supervisor_id: c.assigned_by_supervisor_id || (c.assigned_officer_id ? 902 : null),
    days_unresolved: daysUnresolved,
    is_chronic_overdue: isChronicOverdue,
    evidence_photos: c.evidence_photos && c.evidence_photos.length > 0 ? c.evidence_photos : (c.photo_url ? [c.photo_url] : []),
